...
void AOContDITimeStamp( MenuReturnValues mValues, int idx )
{
	...
	ptrAO_Y_File = fnOpenFileToRead(strAO_XY_FileName, &AO_Y_FileSize); //Call function to open Analog Output file

	/*********************************************/	
	// Load Analog Output Buffer
	/*********************************************/
	ConstructAOBuffer(&dAOBuffer[0], AOOneChanBufSiz, AO_X_FileSize, ptrAOFile, ptrAO_X_File, ptrAO_Y_File); //The first time this should load the full buffer
	...
	while ( !done && !kbhit() )
	{
		...
		if(AOBufferSpaceAvail > AOHalfBuf_Siz)
		{
			...
			ConstructAOBuffer(&dAOBuffer[0], AOOneChanBufSiz, AO_X_FileSize, ptrAOFile, ptrAO_X_File, ptrAO_Y_File);
